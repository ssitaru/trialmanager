...
namespace StudyTranslationManager.entities
{
    ...
    public class Trial
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static List<Trial> GetAllTrials()
        {
            List<Trial> ret = new List<Trial>();

            SQLiteCommand q = new SQLiteCommand("SELECT id FROM trials", Program.SqliteConnection);
            SQLiteDataReader d = q.ExecuteReader();
            while (d.Read())
            {
                ret.Add(new Trial(d.GetInt32(0)));
            }
        ...
        /// <summary>
        /// Delete this trial from the DB
        /// </summary>
        public void Delete()
        {
            if (this.Id != 0)
            {
                SQLiteCommand q = new SQLiteCommand("DELETE FROM trials WHERE id='" + Id + "'", Program.SqliteConnection);
                q.ExecuteNonQuery();
            }
        }
    }
